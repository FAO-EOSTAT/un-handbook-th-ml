## UN Handbook - Chapter 9: Machine learning algorithms for image time series

This repository contains the reproducible analysis for Chapter 9 of the UN Handbook on machine learning algorithms for image time series.
...
```shell
git clone https://github.com/FAO-EOSTAT/un-handbook-th-ml
```

After cloning, navigate into the project directory using `cd un-handbook-th-ml`. All subsequent commands should be executed from within this directory.
...
```shell
docker pull faoeostat/handbook:chp9
```
...
```shell
docker build -t "faoeostat/handbook:chp9" --file docker/Dockerfile .
```
...
docker run \
    ...
    --volume ${PWD}:/home/rstudio/handbook faoeostat/handbook:chp9
```
...
Once you have accessed RStudio in your browser, you need to open the project. The project files are available in the `handbook` directory, which is located in the RStudio user home directory. Navigate to this directory using the Files pane in RStudio. Then double-click on the `th_machine_learning.Rproj` file to open the project. This will configure RStudio with the correct working directory and project settings.
